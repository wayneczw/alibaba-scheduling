...
def main():
    ...
    with open(os.path.join(root, A.machine)) as csvfile:
        ...
        for row in csv_rows:
            ...
            machine_dict.update({row[0]: [cpu_list, mem_list, sorted_resource_capacity_dict, dict(), dict(), float(row[1]), float(row[2]), float(row[3]) / 10, sum_resources]})
        #end for
    #end with
    machine_dict = collections.OrderedDict(sorted(machine_dict.items(), key=lambda t: t[1][-1], reverse=True))
    ...
    # machine_half_count = machine_count * 0.7
    # load current plan
    with open(os.path.join(root, A.instances)) as csvfile:
        csv_rows = csv.reader(csvfile, delimiter=',')
        # machine_priority_set = set()
        for row in csv_rows:
            instance_id = row[0]
            app_id = row[1]
            machine_id = row[2]
            ...
            if machine_id:
                ...
                if check_constraint(machine_dict[machine_id], app_id, app_dict[app_id], interf_dict):
                    # if len(machine_priority_set) < machine_half_count:  # good allocated-instance
                    #    machine_priority_set.add(machine_id)
                    try:
                ...
                for app_id, instance_ids in machine_details[4].items():
                    for instance_id in instance_ids:
                        ...
                        machine_dict, app_dict[app_id], output_csv_rows, success = fix_schedule_instance(
                            machine_dict, machine_id, app_id, app_dict[app_id], interf_dict, output_csv_rows, relax_count=count)  # immediately reschedule
                        # elif count < 3:
                        ...
                        # else:
                        #     machine_dict, app_dict[app_id], output_csv_rows, success = fix_schedule_instance_relax2(
                        #         machine_dict, machine_id, app_id, app_dict[app_id], interf_dict, output_csv_rows)  # immediately reschedule
                        if not success:
                            error_count += 1
                            not_fixed_flag = True
                ...
                if count < 3:
                    if (most_needed_resources == underutilised_capacities) & (len(app_details[4]) > 0):
                        for instance_id in app_details[4]:
                            if check_constraint(machine_details, app_id, app_details, interf_dict, relax_count=count):
...
def fix_schedule_instance(machine_dict, cur_machine_id, app_id, app_details, interf_dict, output_csv_rows, relax_count=0):
    most_needed_resources = [item[0] for item in list(app_details[2].items())[0:3]]
    success = False
    for machine_id, machine_details in machine_dict.items():
        underutilised_capacities = [item[0] for item in list(machine_details[2].items())[0:3]]  # examine the first 3 most underutilised capacities
        if relax_count < 2:
            if (most_needed_resources == underutilised_capacities) & (len(app_details[4]) > 0) & (len(machine_details[4]) == 0):
                if check_constraint(machine_details, app_id, app_details, interf_dict, relax_count=relax_count):
                    success = True
...
def check_constraint(machine_details, app_id, app_details, interf_dict, relax_count=0):
    if machine_details[5] == 32.0:
        E = 16.0
        if relax_count > 10:
            E = E - relax_count * 2.0
    else:
        E = 24.0 - relax_count * 2.0

    if E < 0:
        E = 0.0

    half_cpu = machine_details[5]*E
    half_mem = machine_details[6]*E
    half_disk = machine_details[7]*E

    for i in range(TIME_FRAME):
        if (machine_details[0][i] - app_details[0][i]) < E:  # check cpu list
            return False
        if ((machine_details[1][i] - app_details[1][i]) < 0):  # check mem list
            return False

    if ((machine_details[2]['disk'] - app_details[2]['disk']) < 0):
        return False
